"""
XCom helper for RYUO Python tasks.
Tasks receive XCom functions via the task context (environment variables).

Typical usage inside a RYUO Python task
------------------------------------------
from ryuo.xcom import xcom_push, xcom_pull

# Push a result (reads RYUO_DAG_ID / RYUO_TASK_ID / RYUO_RUN_ID from env)
...
def xcom_push(key: str, value, dag_id: str = None, task_id: str = None, run_id: str = None):
    ...
    Auto-reads dag_id / task_id / run_id from environment variables if not provided:
        RYUO_DAG_ID, RYUO_TASK_ID, RYUO_RUN_ID
    ...
    -------
    dict
        The JSON response from the RYUO API.
    ...
    """
    dag_id = dag_id or os.environ.get("RYUO_DAG_ID", "")
    task_id = task_id or os.environ.get("RYUO_TASK_ID", "")
    run_id = run_id or os.environ.get("RYUO_RUN_ID", "")
    api_key = os.environ.get("RYUO_API_KEY", "ryuo_admin_key")
    base_url = os.environ.get("RYUO_BASE_URL", "http://localhost:3000")

    if not all([dag_id, task_id, run_id]):
        raise ValueError(
            "dag_id, task_id, and run_id are required "
            "(set RYUO_DAG_ID, RYUO_TASK_ID, RYUO_RUN_ID)"
        )
