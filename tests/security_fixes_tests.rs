...
fn test_escape_like_pattern_percent() {
    let result = vortex::db_postgres::escape_like_pattern("100%");
    assert_eq!(result, r"100\%");
}

#[test]
fn test_escape_like_pattern_underscore() {
    let result = vortex::db_postgres::escape_like_pattern("my_dag");
    assert_eq!(result, r"my\_dag");
}

#[test]
fn test_escape_like_pattern_backslash() {
    let result = vortex::db_postgres::escape_like_pattern(r"path\to");
    assert_eq!(result, r"path\\to");
}

#[test]
fn test_escape_like_pattern_all_metacharacters() {
    let result = vortex::db_postgres::escape_like_pattern(r"a%b_c\d");
    assert_eq!(result, r"a\%b\_c\\d");
}

#[test]
fn test_escape_like_pattern_no_special_chars() {
    let result = vortex::db_postgres::escape_like_pattern("normal-dag-id");
    assert_eq!(result, "normal-dag-id");
}

#[test]
fn test_escape_like_pattern_empty() {
    let result = vortex::db_postgres::escape_like_pattern("");
    assert_eq!(result, "");
}
...
fn test_strong_password_accepted() {
    assert!(vortex::db_postgres::validate_password_strength("Str0ng!Pass").is_ok());
    assert!(vortex::db_postgres::validate_password_strength("C0mpl3x@Pwd").is_ok());
    assert!(vortex::db_postgres::validate_password_strength("Ab1!xxxx").is_ok());
}

#[test]
fn test_short_password_rejected() {
    let err = vortex::db_postgres::validate_password_strength("Ab1!").unwrap_err();
    assert!(
...
fn test_no_uppercase_rejected() {
    let err = vortex::db_postgres::validate_password_strength("abcdefg1!").unwrap_err();
...
fn test_no_digit_rejected() {
    let err = vortex::db_postgres::validate_password_strength("Abcdefgh!").unwrap_err();
...
fn test_all_uppercase_with_digits_rejected() {
    ...
    let err = vortex::db_postgres::validate_password_strength("AAAA1111").unwrap_err();
    assert!(
...
fn test_multiple_missing_requirements() {
    let err = vortex::db_postgres::validate_password_strength("aaa").unwrap_err();
...
fn test_vault_argon2id_roundtrip() {
    unsafe { std::env::set_var("VORTEX_SECRET_KEY", "a]bCdEfGhIjKlMnOpQrStUvWxYz12345"); }
    let vault = vortex::vault::Vault::new().expect("Vault::new should succeed");
...
fn test_vault_hex_key_roundtrip() {
    ...
    let vault = vortex::vault::Vault::new().expect("Vault::new with hex key");
...
fn test_vault_cross_key_decryption_fails() {
    unsafe { std::env::set_var("VORTEX_SECRET_KEY", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"); }
    let vault1 = vortex::vault::Vault::new().unwrap();
    let enc1 = vault1.encrypt("test").unwrap();

    unsafe { std::env::set_var("VORTEX_SECRET_KEY", "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"); }
    let vault2 = vortex::vault::Vault::new().unwrap();
