...
#[derive(Parser, Debug)]
#[command(name = "ryuo-cli", about = "RYUO Command Line Interface", version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
impl ApiClient {
    fn new() -> Self {
        let base_url = env::var("RYUO_SERVER_URL").unwrap_or_else(|_| "http://localhost:3000".to_string());
        let api_key = env::var("RYUO_API_KEY").unwrap_or_else(|_| {
            String::new()
        });

        let team_id = env::var("RYUO_TEAM").unwrap_or_else(|_| String::new());
        ...
        if !team_id.is_empty() {
            if let Ok(h) = HeaderValue::from_str(&team_id) {
                headers.insert(reqwest::header::HeaderName::from_static("x-ryuo-team"), h);
            }
        }
